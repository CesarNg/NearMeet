...
import com.hfad.nearmeet.tic_tac_toe.model_game.VictoryLine;
...
public class Board extends FrameLayout {
    ...
    private int MY_SHAPE = Constants.CIRCLE;
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 3; col++) {
                if (field[row][col] != NONE) {
                    shape.drawShape(
                            field[row][col] == Constants.CIRCLE ? Constants.CIRCLE : Constants.X,
                            0,
                            canvas,
                            getRect(row, col),
                            1f);
    ...
    private void drawShape(int row, int col, int shapeType) {
        ...
        isDrawing = true;
        startAnimation(row, col, shapeType == Constants.CIRCLE);
        currentPoint.x = row;
        currentPoint.y = col;
    }

    private void startAnimation(int row, int col, boolean isCircle) {
        currentAnimationShape = isCircle ? Constants.CIRCLE : Constants.X;
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                field[row][col] = isCircle ? Constants.CIRCLE : Constants.X;
